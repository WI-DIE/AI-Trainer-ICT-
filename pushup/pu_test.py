...
# Resize the excellent PNG image to desired size (e.g., 150x100)
desired_width = 150
desired_height = 100
excellent_img = cv2.resize(
    excellent_img, (desired_width, desired_height), interpolation=cv2.INTER_AREA)

# Load the nice PNG image
nice_img_path = "nice.png"
nice_img = cv2.imread(nice_img_path, cv2.IMREAD_UNCHANGED)

# Resize the nice PNG image to desired size (e.g., 150x100)
nice_img = cv2.resize(
    nice_img, (desired_width, desired_height), interpolation=cv2.INTER_AREA)

# Load the bad PNG image
bad_img_path = "bad.png"
bad_img = cv2.imread(bad_img_path, cv2.IMREAD_UNCHANGED)

# Resize the bad PNG image to desired size (e.g., 150x100)
bad_img = cv2.resize(
    bad_img, (desired_width, desired_height), interpolation=cv2.INTER_AREA)

# Now you have excellent_img, nice_img, and bad_img ready to use
...
with mp_pose.Pose(min_detection_confidence=0.8, min_tracking_confidence=0.8) as pose:
    ...
    while video.isOpened() and cap.isOpened():
        ...
        try:
            ...
            # 각 이미지를 불러옵니다. (png 파일 경로는 상황에 맞게 설정)
            excellent_image = cv2.imread('excellent.png')
            nice_image = cv2.imread('nice.png')
            bad_image = cv2.imread('bad.png')

            # pushup logic
            if stage == "down":
                if left_angle < 70 and right_angle < 70:
                    stage = "up"
                elif 70 <= left_angle < 90 and 70 <= right_angle < 90:
                    stage = "up2"
            ...
            if show_excellent and (current_time - excellent_start_time < 2):  # 2초 동안 이미지를 표시
                # 이미지를 원하는 위치에 출력
                image[50:50+excellent_image.shape[0], 50:50 +
                    excellent_image.shape[1]] = excellent_image
            else:
                show_excellent = False

            if show_nice and (current_time - nice_start_time < 2):  # 2초 동안 이미지를 표시
                # 이미지를 원하는 위치에 출력
                image[50:50+nice_image.shape[0], 50:50+nice_image.shape[1]] = nice_image
            else:
                show_nice = False

            if show_bad and (current_time - bad_start_time < 2):  # 2초 동안 이미지를 표시
                # 이미지를 원하는 위치에 출력
                image[50:50+bad_image.shape[0], 50:50+bad_image.shape[1]] = bad_image
            else:
                show_bad = False

            # 기타 OpenCV 창에서 이미지 표시 코드는 아래에 추가
            # 예시:
